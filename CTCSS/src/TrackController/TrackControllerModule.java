...
public class TrackControllerModule extends JFrame {
	...
	public TrackControllerModule() {
		...
		for(index = 0; index < trackControllerList.size(); index++){
			...
			{
				if(trainList.get(count) == 100 && trackControllerList.get(index).blocksControlled.contains(100))
				{
					trackControllerList.get(index).trains++;
					break;
				}
				if(trackControllerList.get(index).blocksControlled.contains(trainList.get(count)))
				{
					trackControllerList.get(index).trains++;
				}
			}
